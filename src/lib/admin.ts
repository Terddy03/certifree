...
export interface CertificationInput {
  ...
  totalReviews?: number;
  tags?: string[];
  type?: "public" | "certifree"; // New: Added for certification type
  courseId?: string; // New: Added for CertiFree certifications
...
export async function createCertification(input: CertificationInput) {
  return supabase.from("certifications").insert({
    // id: input.id, // Removed to allow Supabase to auto-generate UUID
    title: input.title,
    ...
    completion_count: 0,
    tags: input.tags ?? [],
    type: input.type ?? 'public', // New: Default to 'public'
    course_id: input.courseId ?? null, // New: Link to course
  });
}
